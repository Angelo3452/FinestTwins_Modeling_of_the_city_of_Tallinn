...
phase = "First Iterations" # or quantile or Final Iterations
...
i = 1
for c in range(8,numfiles):
    shutil.copy(path_input+"/AM/"+str(c+1)+"/AVG.csv", path_input+"/AM/moeInputs/AVG"+str(c+1)+".csv")
    shutil.copy(path_input+"/PM/"+str(c+1)+"/AVG.csv", path_input+"/PM/moeInputs/AVG"+str(c+1)+".csv")
...
colors = ["#8B8378","#104E8B","#8B2323","#FF3030","#FF8C00","#68228B","#DC143C","#00CED1","#EE1289","#FF1493","#00688B","#104E8B","#B22222","#CD2626","#666666","#0A0A0A"]
Lmaximum = []
nbins = 2000
list_bars = []
...
for i in range(0,numfiles):
    ...
    plt.savefig(path_results+"/It "+str(i+1)+'.png')
    list_n.append(n)
...
for i in range(0,numfiles):
    ...
    for c in range(0, 200):
        plt.boxplot(dict_v[c], positions=[c * step + step / 2], patch_artist=True,
                    boxprops=dict(facecolor="#E9967A", color="#E9967A", linewidth = 0.5), capprops=dict(color="#E9967A", linewidth = 0.5),
                    whiskerprops=dict(color="#E9967A", linewidth = 0.5))
        plt.xticks([])
        plt.legend([])
    if (i % 2) == 0: # if even
...
plt.legend(handles=[handle1,handle2, handle3,handle4], fontsize = 50, prop={"size": 50, "family": "Times New Roman"})
plt.xlim(0, 200)
plt.ylim(0, 2000)
...
plt.savefig(path_results+"/Iterations.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/am/Static Iterations.png')
plt.clf()
...
plt.savefig(path_results+"/Iterations_car_pm.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/pm/Static Iterations.png')
plt.clf()
...
list_DA_pm = list_DA
with open(path_results+"/DA.txt","a") as file1:
...
plt.xticks(ticks = [1,2,3],labels = ["1","2","3"],fontsize=60)
plt.yticks(fontsize=60)
...
plt.savefig(path_results+'/DA_am_bigger_font.png')#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/DA_am.png')
...
plt.savefig(path_results+'/DA_pm_bigger_font.png')#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/DA_pm.png')

##########################################

# All good for cars, but now we have to plot the aTaxi share
...
for c in range(8,numfiles-1):
    #os.remove(path_input + "/AM/moeInputs/AVG" + str(c + 1) + ".csv")
    #os.remove(path_input + "/PM/moeInputs/AVG" + str(c + 1) + ".csv")
    if c < 9:
        shutil.copy(path_input+"/AM/"+str(c+1)+"/am_ivtavg.csv", path_input+"/AM/moeInputs/am_ivtavg_0"+str(c+1)+".csv")
        shutil.copy(path_input+"/PM/"+str(c+1)+"/pm_ivtavg.csv", path_input+"/PM/moeInputs/pm_ivtavg_0"+str(c+1)+".csv")
    else:
        shutil.copy(path_input+"/AM/"+str(c+1)+"/am_ivtavg.csv", path_input+"/AM/moeInputs/am_ivtavg_"+str(c+1)+".csv")
        shutil.copy(path_input+"/PM/"+str(c+1)+"/pm_ivtavg.csv", path_input+"/PM/moeInputs/pm_ivtavg_"+str(c+1)+".csv")
for c in range(8,numfiles-1):
    if c < 9:
        shutil.copy(path_input+"/AM/"+str(c+1)+"/am_wtavg.csv", path_input+"/AM/moeInputsWt/am_wtavg_0"+str(c+1)+".csv")
        shutil.copy(path_input+"/PM/"+str(c+1)+"/pm_wtavg.csv", path_input+"/PM/moeInputsWt/pm_wtavg_0"+str(c+1)+".csv")
    else:
        shutil.copy(path_input+"/AM/"+str(c+1)+"/am_wtavg.csv", path_input+"/AM/moeInputsWt/am_wtavg_"+str(c+1)+".csv")
        shutil.copy(path_input+"/PM/"+str(c+1)+"/pm_wtavg.csv", path_input+"/PM/moeInputsWt/pm_wtavg_"+str(c+1)+".csv")


for skim_am_path, wt_am_path, wt_pm_path, skim_pm_path, activity_schedule_path in zip([path_input+'/AM/moeInputs/' + file for file in sorted(os.listdir(path_input+"/AM/moeInputs"))],[path_input+'/AM/moeInputsWt/' + file for file in sorted(os.listdir(path_input+"/AM/moeInputsWt"))],[path_input+'/PM/moeInputsWt/' + file for file in sorted(os.listdir(path_input+"/PM/moeInputsWt"))],
                                                              [path_input+'/PM/moeInputs/' + file for file in sorted(os.listdir(path_input+"/PM/moeInputs"))], [path_input+'/DAS/moeInputs/' + file for file in sorted(os.listdir(path_input+"/DAS/moeInputs"))]):
    skim_am = pd.read_csv(skim_am_path, index_col=0)
    skim_pm = pd.read_csv(skim_pm_path, index_col=0)
    wt_am = pd.read_csv(wt_am_path, index_col = 0)
    wt_pm = pd.read_csv(wt_pm_path, index_col=0)
    activity_schedule=pd.read_csv(activity_schedule_path, header=0, index_col=False)#, names=["person_id","tour_no","tour_type","stop_no","stop_type","stop_location",
                                                                   #"stopZone","stop_mode","primary_stop","arrival_time","departure_time",
                                                                   #"prev_stop_location","prev_stopZone","prev_stop_departure_time","pid"])
    ...
    skim_am["avg"]=skim_am["avg"]*60
    skim_pm["avg"]=skim_pm["avg"]*60
    wt_am["wt"]=wt_am["wt"]*60
    wt_pm["wt"]=wt_pm["wt"]*60
    am = skim_am.merge(wt_am, on='origin', how='left')
    pm = skim_pm.merge(wt_pm, on='origin', how='left')
    am['tt'] = am['avg']+am['wt']
    am['avg'] = am['tt']
    am = am.drop(columns=["wt","tt"])
    pm['tt'] = pm['avg']+pm['wt']
    pm['avg'] = pm['tt']
    pm = pm.drop(columns=["wt","tt"])
    # skim matrixes to tables - 2 columns - it works as intended
    skim_am_table = am#skim_am
    skim_pm_table = pm#skim_pm
    ...
    skim_am_table = skim_am_table[skim_am_table["origin_aimsun"] != skim_am_table["destination_aimsun"]]
    skim_pm_table = skim_pm_table[skim_pm_table["origin_aimsun"] != skim_pm_table["destination_aimsun"]]
    activity_schedule_am = activity_schedule[
        (activity_schedule["arrival_time"] >= 7.25) & (activity_schedule["arrival_time"] <= 9.75)]
    activity_schedule_am = activity_schedule_am[
        (activity_schedule_am["stop_mode"] == "Taxi")]
    activity_schedule_pm = activity_schedule[
        (activity_schedule["arrival_time"] >= 16.25) & (activity_schedule["arrival_time"] <= 18.75)]
    activity_schedule_pm = activity_schedule_pm[
        (activity_schedule_pm["stop_mode"] == "Taxi")]
    ...
    skim_am_table_count = skim_am_table.groupby(["origin_aimsun", "destination_aimsun"]).sum().reset_index()
    skim_pm_table_count = skim_pm_table.groupby(["origin_aimsun", "destination_aimsun"]).sum().reset_index()
    # count number of unique O-D couples for number of trips
    len_trips_number_am.append(len(activity_schedule_am_count))
    len_trips_number_pm.append(len(activity_schedule_pm_count))
    len_tt_couples_am.append(len(skim_am_table_count))
    ...
    # skimindex.to_csv(r"skimindex.csv")
    # join skim matrix and table with number of trips received from activity schedule using O-D couples
    am_concated = pd.concat([skim_am_table.set_index(["origin_aimsun", "destination_aimsun"]),
                             activity_schedule_am_count.set_index(["prev_stopZone", "stopZone"])], axis=1)
    pm_concated = pd.concat([skim_pm_table.set_index(["origin_aimsun", "destination_aimsun"]),
                             activity_schedule_pm_count.set_index(["prev_stopZone", "stopZone"])], axis=1)
    # am_concated.to_csv(r"concated_start"+str(i)+".csv")
...
trips_number_am=trips_number_am.fillna(0)
trips_number_pm=trips_number_pm.fillna(0)
travel_times_am=travel_times_am.fillna(0)
travel_times_pm=travel_times_pm.fillna(0)
...
list_DA = []
kws = dict(histtype= "stepfilled")
colors = ["#8B8378","#104E8B","#8B2323","#FF3030","#FF8C00","#68228B","#DC143C","#00CED1","#702963","#FF1493","#00688B","#104E8B","#B22222","#CD2626","#666666","#0A0A0A"]#["green","blue","gray","red"]#
#colors = ["#000000","#000000","#000000","#000000","#000000","#000000","#000000","#000000","#EE1289","#FF1493","#00688B","#104E8B","#B22222","#CD2626","#666666","#0A0A0A"]#["green","blue","gray","red"]#

Lmaximum = []
nbins = 273
list_bars = []

for i in range(1, numfiles-9):
    if i == 1:
        Lmaximum.append(max(am_multipl_plot[am_multipl_plot.iloc[:, i-1] != 0].iloc[:, i-1]))
    Lmaximum.append(max(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i]))

for i in range(0,numfiles-9):
    maximum = max(Lmaximum)
    n, bins, _ = plt.hist(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum))
    plt.clf()
    plt.hist(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i))
...
dict_v = {key: list(value) for key in keys}
list_v = []
for i in range(0,numfiles-9): ### deviation only for the first 4, as we are plotting them for now
    for c in range(0,nbins): ### one list of 4 for each bin
        dict_v[c].append(list_n[i][c])#(list_area[i][c])
...
#handle6=mpatches.Patch(color=colors[5], label="Iteration 6", alpha = 0.4)
#handle7=mpatches.Patch(color=colors[6], label="Iteration 7", alpha = 0.4)
#handle8=mpatches.Patch(color=colors[7], label="Iteration 8", alpha = 0.4)
#handle9=mpatches.Patch(color=colors[8], label="Iteration 9", alpha = 0.4)
#handle10=mpatches.Patch(color=colors[9], label="Iteration 10", alpha = 0.4)
#handle11=mpatches.Patch(color=colors[10], label="Iteration 11", alpha = 0.4)
#handle12=mpatches.Patch(color=colors[11], label="Iteration 12", alpha = 0.4)
#handle13=mpatches.Patch(color=colors[12], label="Iteration 13", alpha = 0.4)
#handle14=mpatches.Patch(color=colors[13], label="Iteration 14", alpha = 0.4)
#handle15=mpatches.Patch(color=colors[14], label="Iteration 15", alpha = 0.4)
...
dict_v = {key: list(value) for key in keys}
list_v = []
for i in range(0,2): ### deviation only for the first 4, as we are plotting them for now
    for c in range(0,nbins): ### one list of 4 for each bin
        dict_v[c].append(list_n[i][c])#(list_area[i][c])
plt.figure(figsize=(20, 16))
for i in range(0,2):
    ...
    if i % 2: # if odd
        plt.hist(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i), edgecolor=colors[i], **kws, fill = False,linewidth=3,zorder=10, linestyle = 'dotted')
#plt.legend(["1","2","3","4","5"],fontsize=28, prop={"size": 22, "family": "Times New Roman"})
handle1=mpatches.Patch(color=colors[0], label="Iteration 9", alpha = 0.4)
handle2=mpatches.Patch(color=colors[1], label="Iteration 10", alpha = 0.4)
handle3=mpatches.Patch(color=colors[2], label="Iteration 7", alpha = 0.4)
handle4=mpatches.Patch(color=colors[3], label="Iteration 8", alpha = 0.4)
handle5=mpatches.Patch(color=colors[4], label="Iteration 5", alpha = 0.4)
handle6=mpatches.Patch(color=colors[5], label="Iteration 6", alpha = 0.4)
handle7=mpatches.Patch(color=colors[6], label="Iteration 7", alpha = 0.4)
handle8=mpatches.Patch(color=colors[7], label="Iteration 8", alpha = 0.4)

plt.legend(handles=[handle1,handle2], fontsize = 50, prop={"size": 50, "family": "Times New Roman"})
plt.xlim(0, 200)
plt.ylim(0, 1500)
#plt.xticks(ticks = [10,50,100,150,200],labels = ["10","50","100","150","200"], fontsize=50)
plt.yticks(fontsize=50)
plt.ylabel('Observation in each bin',fontsize=50, fontname='Times New Roman')
plt.xlabel('Intervals',fontsize=50, fontname='Times New Roman')
plt.title('',fontsize=60, fontname='Times New Roman')
plt.savefig(path_results+"/aTaxi_Iterations_starting_5min.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/am/Static Iterations.png')
plt.clf()
...
dict_v = {key: list(value) for key in keys}
list_v = []
for i in range(9,numfiles): ### deviation only for the first 4, as we are plotting them for now
    for c in range(0,nbins): ### one list of 4 for each bin
        dict_v[c].append(list_n[i][c])#(list_area[i][c])
plt.figure(figsize=(20, 16))
for i in range(9,numfiles):
    maximum = max(Lmaximum)
    step = maximum/nbins
    n, bins, _ = plt.hist(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum), fill = False, edgecolor="white")
    for c in range(0, 200):
    ...
    if i % 2: # if odd
        plt.hist(am_multipl_plot[am_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i), edgecolor=colors[i], **kws, fill = False,linewidth=3,zorder=10, linestyle = 'dotted')
#plt.legend(["1","2","3","4","5"],fontsize=28, prop={"size": 22, "family": "Times New Roman"})
handle1=mpatches.Patch(color=colors[8], label="Iteration 9", alpha = 0.4)
handle2=mpatches.Patch(color=colors[9], label="Iteration 10", alpha = 0.4)
handle3=mpatches.Patch(color=colors[10], label="Iteration 11", alpha = 0.4)
handle4=mpatches.Patch(color=colors[11], label="Iteration 12", alpha = 0.4)
handle5=mpatches.Patch(color=colors[12], label="Iteration 13", alpha = 0.4)
handle6=mpatches.Patch(color=colors[13], label="Iteration 14", alpha = 0.4)
handle7=mpatches.Patch(color=colors[14], label="Iteration 15", alpha = 0.4)


plt.legend(handles=[handle1,handle2, handle3,handle4,handle5,handle6, handle7], fontsize = 50, prop={"size": 50, "family": "Times New Roman"})
plt.xlim(0, 200)
plt.ylim(0, 2000)
#plt.xticks(ticks = [10,50,100,150,200],labels = ["10","50","100","150","200"], fontsize=50)
plt.yticks(fontsize=50)
...
plt.xlabel('Intervals',fontsize=50, fontname='Times New Roman')
plt.title('',fontsize=60, fontname='Times New Roman')
plt.savefig(path_results+"/aTaxi_Iterations_qt_1min.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/am/Static Iterations.png')
plt.clf()
...
for i in range(0,2):
    ...
    if i % 2: # if odd
        plt.hist(pm_multipl_plot[pm_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i), edgecolor=colors[i], **kws, fill = False,linewidth=3,zorder=10, linestyle = 'dotted')
#plt.legend(["1","2","3","4","5"],fontsize=28, prop={"size": 22, "family": "Times New Roman"})
handle1=mpatches.Patch(color=colors[0], label="Iteration 7", alpha = 0.4)
handle2=mpatches.Patch(color=colors[1], label="Iteration 8", alpha = 0.4)
handle3=mpatches.Patch(color=colors[2], label="Iteration 3", alpha = 0.4)
handle4=mpatches.Patch(color=colors[3], label="Iteration 4", alpha = 0.4)
handle5=mpatches.Patch(color=colors[4], label="Iteration 5", alpha = 0.4)
handle6=mpatches.Patch(color=colors[5], label="Iteration 6", alpha = 0.4)
...
plt.xlabel('Intervals',fontsize=50, fontname='Times New Roman')
plt.title('',fontsize=60, fontname='Times New Roman')
plt.savefig(path_results+"/aTaxi_Iterations_starting_pm_5min.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/am/Static Iterations.png')
plt.clf()
...
plt.xlabel('Intervals',fontsize=50, fontname='Times New Roman')
plt.title('',fontsize=60, fontname='Times New Roman')
plt.savefig(path_results+"/aTaxi_Iterations_pm_qt_1min.png")#(r'/home/angelo/Scripting (Static)/New/Script-plots and MoE/histogram/am/Static Iterations.png')
plt.clf()
...
for i in range(1,len(list_area)):
    ...
    maximum = max(Lmaximum)
    #maximum = max(max(pm_multipl_plot[pm_multipl_plot.iloc[:, i-1] != 0].iloc[:, i-1]),max(pm_multipl_plot[pm_multipl_plot.iloc[:, i] != 0].iloc[:, i]))
    plt.hist(pm_multipl_plot[pm_multipl_plot.iloc[:, i-1] != 0].iloc[:, i-1], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i-1))
    plt.hist(pm_multipl_plot[pm_multipl_plot.iloc[:, i] != 0].iloc[:, i], alpha=0.4, bins=nbins, range=(0, maximum),label=str(i))
    #plt.bar(list_bins[i - 1][:-1], list_intersection[i - 1], width=list_bins[i][i] - list_bins[i - 1][i-1],label=str(i-1), alpha = 0.5)
    #plt.bar(list_bins[i][:-1], list_intersection[i], width=list_bins[i][i] - list_bins[i - 1][i - 1], label=str(i), alpha = 0.5)
    plt.legend([str(i), str(i+1)],fontsize=60, prop={"size": 60, "family": "Times New Roman"})
...
print(list_DA_am)
print(list_DA_pm)
colors = ['#0000FF','#0000FF','#0000FF','#0000FF','#0000FF','#0000FF','#0000FF','#FF0000','#0000FF','#0000FF','#0000FF','#0000FF','#0000FF','#0000FF']
plt.clf()
plt.figure(figsize=(20, 16))
plt.bar(range(1,2),list_DA_am[0], color = colors)
plt.xticks(ticks = [1],labels = ["1-2"],fontsize=60,rotation=90)
plt.yticks(fontsize=60)
plt.ylabel('MoE',fontsize=60, fontname='Times New Roman')
plt.xlabel('ΔA',fontsize=60, fontname='Times New Roman')
plt.title('',fontsize=60, fontname='Times New Roman')
...
plt.clf()
plt.figure(figsize=(20, 16))
plt.bar(range(1,2),list_DA_pm[0], color = colors)
plt.xticks(ticks = [1],labels = ["1-2"],fontsize=60,rotation=90)
plt.yticks(fontsize=60)
plt.ylabel('MoE',fontsize=60, fontname='Times New Roman')
plt.xlabel('ΔA',fontsize=60, fontname='Times New Roman')
